...
class Entry:
    ...
    @staticmethod
    def create(stack_id, title, content="", tags=""):
        db = get_db()
        db.execute(
            "INSERT INTO entry (stack_id, title, content, tags) VALUES (?, ?, ?, ?)",
    ...
    @staticmethod
    def update(entry_id, title, content, tags):
        db = get_db()
        db.execute(
            "UPDATE entry SET title = ?, content = ?, tags = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
